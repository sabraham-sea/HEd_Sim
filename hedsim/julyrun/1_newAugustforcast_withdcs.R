...
cap_traffic<-cap_data_daily%>%filter(date <= datechk )%>%group_by(cap_id)
...
pred_forecast_cpl<-function(capid,datechk,period_rng,futuredate1,futuredate2,thisdate,category_name)
{
  
  set.seed(1234)
  # Make traffic forecasts
  cap_traffic<-cap_data_daily%>%filter(cap_id==capid & category_name== category_name)%>%
    filter(date <= datechk )%>% 
  ...
  cap_views<-cap_data_daily%>%
    filter(cap_id==capid & category_name==category_name)%>%filter(date <= datechk )%>%
...
# cc<-head(caplist,n=n1)%>%as.data.frame()%>%rename(cap_id= ".")
 ...
 n1<-NROW(catlist1)
 cc1<-head(catlist$cap_id,n=n1)%>%as.data.frame()%>%rename(cap_id= ".")
 cc2<-head(catlist$category_name,n=n1)%>%as.data.frame()%>%rename(category_name= ".")
...
clusterExport(cl,list('cc1','cap_data_daily','datechk','period_rng','futuredate1','futuredate2','thisdate','cc2'), envir = environment())
system.time(forcast_leads <- parLapply(cl,cc1$cap_id,pred_forecast_cpl,datechk,period_rng,futuredate1,futuredate2,thisdate,cc2$category_name))
