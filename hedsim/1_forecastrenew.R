...
cap_data_daily<-trf_daily_clean%>%filter(date<='2020-04-30')%>%dplyr::select(cap_id, school_name,date,degree_name,subject_name,accepted_cpl_leads,accepted_cpl_appviews,accepted_cpc_clicks,accepted_cpc_leads,
...
billable_leadrevenue_newrun<-left_join(forcast_leads1,pricelist,by="cap_id")%>%mutate(forcast_billable=forcast*accepted_revenue)%>%filter(!is.na(accepted_revenue))

saveRDS(billable_leadrevenue_Maypred,"billable_leadrevenue_newrun.RDS")
