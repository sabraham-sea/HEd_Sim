...
CDM_traffic_clean<-CDM_traffic%>%filter(cap_id != '')%>%arrange(date,cap_id,subject_name)
...
month_traffic_org1<-CDM_traffic_clean%>%group_by(date,cap_id,subject_name,school_name)%>%summarise(tottraff=sum(dcs_traffic))%>%arrange(date,cap_id,subject_name)
...
combined_daily<-left_join(month_traffic_lat1,month_traffic_org1,by=c("cap_id","school_name","date","subject_name"))%>%arrange(date,cap_id,category_name)
combined_daily$impression<-ifelse((combined_daily$tottraff<combined_daily$cpl_views), NA,combined_daily$tottraff)
saveRDS(combined_daily,"trf_daily_final_chk.RDS")
...
combined_daily<-readRDS("trf_daily_final_chk.RDS")
...
system.time(imputed_daily<-mice(data.frame(combined_daily$impression,combined_daily$cap_id),m=5,maxit=5,method="pmm"))
daily_new=complete(imputed_daily,5)
daily_new<-daily_new%>%rename('imptraf'='combined_daily.impression','cap_id'='combined_daily.cap_id')
...
saveRDS(trf_daily_final,"trf_daily_final_DCS.RDS")
