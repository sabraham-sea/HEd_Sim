...
clust2_caps<-clusterlist$`2`%>%rownames_to_column("id")%>% separate(id, into = c("cap"))%>%
  distinct(cap)
clust3_caps<-clusterlist$`3`%>%rownames_to_column("id")%>% separate(id, into = c("cap"))%>%
  distinct(cap)
clust4_caps<-clusterlist$`4`%>%rownames_to_column("id")%>% separate(id, into = c("cap"))%>%
  distinct(cap)
clust5_caps<-clusterlist$`5`%>%rownames_to_column("id")%>% separate(id, into = c("cap"))%>%
  distinct(cap)
clust6_caps<-clusterlist$`6`%>%rownames_to_column("id")%>% separate(id, into = c("cap"))%>%
  distinct(cap)
