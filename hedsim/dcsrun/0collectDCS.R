...
month_traffic_org1<-CDM_traffic_clean%>%group_by(date,cap_id,subject_name)%>%summarise(tottraff=sum(dcs_traffic))%>%arrange(date,cap_id,subject_name)
...
combined_daily<-left_join(month_traffic_lat1,month_traffic_org1,by=c("cap_id","date","subject_name"))%>%arrange(date,cap_id,category_name)
