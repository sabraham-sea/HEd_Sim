...
require(prophet)
 pricelist<-combined_daily%>%select(cap_id,accepted_revenue,billable_revenue,revised_revenue,date)%>%filter(date=='2020-04-01' & accepted_revenue>0)%>%distinct()
 caplist<-pricelist$cap_id
...
pred_forecast<-function(capid,datechk,period_rng)
{
    ...
    cap_traffic<-combined_daily%>%filter(cap_id==capid)%>%
        filter(date <= datechk )%>% 
        rename(y=dcs_traffic, ds=date)
    ...
    cap_views<-combined_daily%>%
        filter(cap_id==capid)%>%filter(date <= datechk )%>%
        rename(y=accepted_cpl_appviews, ds=date)
    ...
    leads_cpl<-combined_daily%>%
        filter(cap_id==capid)%>%filter(date <= datechk )%>%
        rename(y=accepted_cpl_leads, ds=date)
    ...
    cpl_daily<-add_regressor(cpl_prf,name='dcs_traffic')
    cpl_daily<-add_regressor(cpl_prf,name='accepted_cpl_appviews')
    ...
    df_all <- leads_df%>%left_join(dplyr::select(trf_daily_forecast,yhat,ds), by = 'ds')%>%rename(dcs_traffic=yhat)
    df_all <- df_all  %>% left_join(dplyr::select(views_daily_forecast, yhat,ds), by = 'ds')%>%rename(accepted_cpl_appviews=yhat)
